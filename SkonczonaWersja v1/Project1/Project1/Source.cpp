...
int main(void)
{
	...
	plik.close();
	ofstream plik2;
	plik2.open("ranking.txt", ios::in | ios::out | ios::binary);
	int widht = 1000;
	int height = 700;
	...
	ALLEGRO_FONT * czcionka2 = al_load_ttf_font("do.ttf", 40, 0);
	...
	koniecc = al_load_bitmap("grafiki/menu/koniecc.bmp");
	al_convert_mask_to_alpha(koniecc, al_map_rgb(255, 0, 255));
	...
	while (!done)
	{
			...
			while (!ekrangry) {
				...
				while (!zamknij)
				{
					...
					if (!kolizja)
					{
						...
						// zycie
						if ( hp >= 90) { al_draw_bitmap(zycie10, 320, 660, 0); }
					...
					if (poziom) {
						...
						al_draw_textf(czcionka, al_map_rgb(0, 255, 0), 900, 90, ALLEGRO_ALIGN_CENTRE, "czas=%f ", czas/60);
		...
		ranking:
			...
			if (rank[1]<czas && czas<rank[0])
			{
				rank[2] = rank[1];
				rank[1] = czas;

			}
			if (rank[2]<czas && czas<rank[1])
			{
				rank[2] = czas;
			}
			plik2 << rank[0] << " " << rank[1] << " " << rank[2];
			bool opcje = false;
			while (!opcje) {
				...
				if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
					if (ev.mouse.button & 2) {
						...
						if (pos_x > 748 && pos_x < 906 && pos_y> 541 && pos_y < 583) {
				...
				al_draw_textf(czcionka2, al_map_rgb(0, 255, 0), 400, 240, 0, "%d", rank[2]);
				al_draw_textf(czcionka2, al_map_rgb(0, 255, 0), 400, 335, 0, "%d", rank[1]);
				al_draw_textf(czcionka2, al_map_rgb(0, 255, 0), 400, 430, 0, "%d", rank[0]);
			...
			while (!zamknij)
			{
				...
				if (!kolizja)
				{
					...
					if (!yyy2) {
						...
						al_draw_bitmap(klucz, 384, 448, 0);
					}

					if (!prezentt2) {

						al_draw_bitmap(prezent, 480, 160, 0);
					}
					...
					if (gracz_x >= 672 && gracz_x <= 704 && gracz_y >= 416 && gracz_y < 448) {
					...
					if (gracz_x >= 352 && gracz_x <= 384 && gracz_y >= 448 && gracz_y < 480) {

						kluczyk2++;
						yyy2 = true;
					}

					if (gracz_x >= 480 && gracz_x <= 512 && gracz_y >= 160 && gracz_y < 192) {
				...
				if (poziom) {
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 230, ALLEGRO_ALIGN_CENTRE, "x=%d y=%d h=%f ", gracz_x, gracz_y, hp);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 260, ALLEGRO_ALIGN_CENTRE, "t=%f  ", tablica);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 280, ALLEGRO_ALIGN_CENTRE, "klucz=%f ", kluczyk2);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 360, ALLEGRO_ALIGN_CENTRE, "prezent=%f ", prezencik2);
					al_draw_textf(czcionka, al_map_rgb(0, 255, 0), 900, 90, ALLEGRO_ALIGN_CENTRE, "czas=%f ", czas/60);
			...
			while (!zamknij)
			{
				...
				if (!kolizja)
				{
					...
					if (!yyy3) {
						...
						al_draw_bitmap(klucz, 416, 352, 0);
					}

					if (!prezentt3) {

						al_draw_bitmap(prezent, 128, 32, 0);
					}
					...
					if (gracz_x >= 416 && gracz_x <= 448 && gracz_y >= 224 && gracz_y < 256) {
					...
					if (gracz_x >= 416 && gracz_x <= 448 && gracz_y >= 352 && gracz_y < 384) {

						kluczyk3++;
					}

					if (gracz_x >= 128 && gracz_x <= 160 && gracz_y >= 32 && gracz_y < 64) {
				...
				if (poziom) {
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 230, ALLEGRO_ALIGN_CENTRE, "x=%d y=%d h=%f ", gracz_x, gracz_y, hp);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 260, ALLEGRO_ALIGN_CENTRE, "t=%f  ", tablica);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 280, ALLEGRO_ALIGN_CENTRE, "klucz=%f ", kluczyk3);
					al_draw_textf(font24, al_map_rgb(0, 255, 0), 900, 360, ALLEGRO_ALIGN_CENTRE, "prezent=%f ", prezencik3);
					al_draw_textf(czcionka, al_map_rgb(0, 255, 0), 900, 90, ALLEGRO_ALIGN_CENTRE, "czas=%f ", czas/60);
				...
				if (prezencik3 > 1 && kluczyk3 > 1) {
					if (gracz_x == 448 && gracz_y < 486 && gracz_y> 480) {
